...
% Pop up a window 
 [files,directory] = uigetfile('Multiselect','on','\\FS1\WisorData\*.edf','PCASCOREBATCHMODE!  Please select the .edf file(s) you would like to autoscore');  %last parameter sent to uigetfile ('*.edf*) specifies that only edf files will be displayed in the user interface.
...
for i=1:length(files)
	files{i}
	[predicted_score,kappa(i),global_agreement(i),wake_agreement(i),SWS_agreement(i),REM_agreement(i)]=classify_usingPCA([directory files{i}],method,signal,restrict,training_start_time,training_end_time,trials,writefile,output_directory);
