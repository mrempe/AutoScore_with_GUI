function kappa=compute_kappa(v1,v2,num_value_wake,num_value_sws,num_value_rem)
...
%
	...
	if iscell(v1) & iscell(v2)
		...
		% Set up the Interobserver Variation table
		both_wake = length(find(v1==num_value_wake & v2==num_value_wake));
		both_SWS  = length(find(v1==num_value_sws  & v2==num_value_sws));
		both_REM  = length(find(v1==num_value_rem  & v2==num_value_rem));

		W1_S2 = length(find(v1==num_value_wake & v2==num_value_sws));
		W1_R2 = length(find(v1==num_value_wake & v2==num_value_rem));
		S1_W2 = length(find(v1==num_value_sws  & v2==num_value_wake));
		S1_R2 = length(find(v1==num_value_sws  & v2==num_value_rem));
		R1_W2 = length(find(v1==num_value_rem  & v2==num_value_wake));
		R1_S2 = length(find(v1==num_value_rem  & v2==num_value_sws));
	end
