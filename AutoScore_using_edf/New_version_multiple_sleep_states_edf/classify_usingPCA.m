...
size(epochs)
	...
	disp(['There were ', num2str(length(find(SleepState==8))) ' epochs that were not scored.'])
...
if isempty(number_value_for_REMS)
 ...
 %        SleepState(i)=3;                  % call transitions wake
...
if fully_scored == 0   %using all scored epochs as training data
	original_scored_rows = find(SleepState ~=8 & SleepState ~=-1 );               % file has not been fully scored. Exclude artefact epochs. 
	...
	original_scored_rows = original_scored_rows(find(SleepState(original_scored_rows)~=-1));  % Only use the training data that doesn't have artefacts. 
...
if trials.number > 1
	...
	for i=2:trials.number+1
		...
		while length(find(SleepState(scored_rows{i})==number_value_for_REMS))<num_REMS_episodes_desired 
...
for j=1:M
 ...
 if strcmp(method,'RandomForest')
	...
	predicted_sleep_state(find(SleepState==-1)) = -1; % reset artifact epochs back to artifact
 end 
	...
	if rescore_REM_in_wake
		for i=1:epochs_in_REM_window %If REM occurs in first 30 seconds of recording, rescore it as wake
			if predicted_sleep_state(i,j)==number_value_for_REMS
				predicted_sleep_state(i,j)=number_value_for_wake;
				REM_rescore_counter = REM_rescore_counter+1;
			end
		end
		REM_locs = find(predicted_sleep_state(:,j)==number_value_for_REMS);
		...
		for i=1:length(REM_locs)
			if predicted_sleep_state(REM_locs(i)-epochs_in_REM_window:REM_locs(i)-1,j)==zeros(epochs_in_REM_window,1)
	       	predicted_sleep_state(REM_locs(i),j) = number_value_for_wake;  %set that epoch to wake
	       	REM_rescore_counter = REM_rescore_counter+1;
	   		end
		end
	end
	...
	if rescore_REM_using_EMG & length(find(predicted_sleep_state(:,j)==number_value_for_REMS)) > 0
		% --- If a REM episode ends without the EMG changing, extend it until EMG changes --
		predicted_sleep_state(:,j)=rescoreREM_usingEMG(Feature,predicted_sleep_state(:,j),number_value_for_wake,number_value_for_REMS);  
	...
	if fully_scored
	kappa(j) = compute_kappa(SleepState(non_artefact_indices),predicted_sleep_state(non_artefact_indices,j),number_value_for_wake,number_value_for_SWS,number_value_for_REMS);
	[global_agreement(j),wake_agreement(j),SWS_agreement(j),REM_agreement(j)] = compute_agreement(SleepState(non_artefact_indices),predicted_sleep_state(non_artefact_indices,j),...
		 																						  number_value_for_wake,number_value_for_SWS,number_value_for_REMS);
	else
		% compute agreement stats on only those epochs that were scored by hand
		kappa(j) = compute_kappa(SleepState(scored_rows{j}),predicted_sleep_state(scored_rows{j},j),number_value_for_wake,number_value_for_SWS,number_value_for_REMS);
...
if strcmp(method,'NaiveBayes')
	figure
	gscatter(PCAvectors(scored_rows{best_trial},1),PCAvectors(scored_rows{best_trial},2),SleepState(scored_rows{best_trial}),[1 0 0; 0 0 1; 1 .5 0; 0 1 0; 0.5 1 0; 0 1 0.5],'osdx+*h<');


	if length(unique_sleep_states) >= 3  % if at least three states are present, draw lines on the scatterplot to indicate the states 
...
subplot(1,2,1)
% grouping
wake_locs_human = find(SleepState==number_value_for_wake);
SWS_locs_human  = find(SleepState==number_value_for_SWS);
REM_locs_human  = find(SleepState==number_value_for_REMS);
...
subplot(1,2,2)
wake_locs_machine = find(predicted_sleep_state==number_value_for_wake);
SWS_locs_machine  = find(predicted_sleep_state==number_value_for_SWS);
REM_locs_machine  = find(predicted_sleep_state==number_value_for_REMS);
