...
dot_locs   = strfind(textdata{1,1},'.');


if ~isempty(slash_locs)
  month_loc = slash_locs(1)-2:slash_locs(1)-1; 
  day_loc = slash_locs(1)+1:slash_locs(1)+2;  % day info is found in 2 positions to the left of second slash
  year_loc = slash_locs(2)+1:slash_locs(2)+4;
elseif ~isempty(dot_locs)
  day_loc = dot_locs(1)-2:dot_locs(1)-1;  % day info is found in 2 positions to the left of first slash
  month_loc = dot_locs(1)+1:dot_locs(1)+2;
  year_loc = dot_locs(2)+1:dot_locs(2)+4;
else
  error('I couldn''t find a slash or a dot in the timestamp, so I couldn''t parse the date.')
end
...
% Hour, Minute, Seconds
TimeStampMatrix(:).Hour = zeros(length(textdata),1);  %allocate first
TimeStampMatrix(:).Minute =zeros(length(textdata),1);
TimeStampMatrix(:).Second = zeros(length(textdata),1);
