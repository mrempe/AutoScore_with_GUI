...
classdef XL
	...
	methods (Static)
		...
		function range = setCells( sheet, position, data, clr, autofit )
			%size(position)
			if numel(position)==1
				disp('position has only one element!!')
			end
			...
			%range = get(sheet,'Range',[ upper(Units.hexavigesimal(position(1))) num2str(position(2)) ':'  upper(Units.hexavigesimal(position(1) + size(data,2) - 1)) num2str(position(2) + size(data,1) -1) ]);
