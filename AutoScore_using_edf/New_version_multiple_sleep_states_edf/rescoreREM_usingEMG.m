function NewSleepState = rescoreREM_usingEMG(Feature,SleepState,num_value_wake,num_value_REM) 
	%
	...
	EMG = Feature(:,5);
	REM_epochs = find(SleepState==num_value_REM);
	...
	runs = contiguous(SleepState,num_value_REM);    % find all runs of REMS
	runs = runs{1,2};
	...
	for i=1:size(runs,1)
		...
		if runs(i,1) ~= runs(i,2) & runs(i,2) < size(SleepState,1)  % if REMS episode is more than one epoch (and it's not at end of recording)
			if SleepState(runs(i,2)+1)==num_value_wake  % if the REM streak ends with Wake
				j=1;  %was j=i;
				while (runs(i,2)+j < length(EMG) & SleepState(runs(i,2)+j) ~= 1 & (EMG(runs(i,2)+j) ) < EMG_avg_for_REM_run + 1*std_for_REM_run)
					disp('rescoreREM_usingEMG rescored an epoch to REM')
					SleepState(runs(i,2)+j) = num_value_REM;
					j=j+1;
				end
			end
		end
		...
		if runs(i,1) == runs(i,2) & runs(i,2) < size(SleepState,1)   %runs of only 1 epoch, not last epoch
			if SleepState(runs(i,2)+1)==num_value_wake   %if the next epoch is Wake 
				j=1;
